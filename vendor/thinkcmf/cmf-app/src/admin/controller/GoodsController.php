...
class GoodsController extends AdminBaseController
{
    ...
    public function quote_goods(){
        ...
        $goodsModel=db('goods');
        $list=$goodsModel->where('quote_status',1)->where($where)->order('id DESC')->paginate(50,false,['query'=>$param])->each(function($item,$key){
            $isset_parts=db('goods_parts')->where('goods_id',$item['id'])->count();
            if($isset_parts){
                $item['parts'] = 1;
            }else{
	...
	public function offer_log(){
		...
		$offerData = $offerModel->where('goods_id',$param['id'])->order('id desc')->paginate(20);
	...
	public function shangjia(){
        ...
        $result=json_decode($res->out,true);
		if($result['resultFlag']=='Y'){
            $goodsModel->where('id',$param['id'])->update(['zt'=>2]);
            $this->success($result['resultMessage']);
        }else{
            $this->error($result['resultMessage']);
        }
