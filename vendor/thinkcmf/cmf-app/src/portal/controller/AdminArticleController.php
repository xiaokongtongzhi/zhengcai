...
class AdminArticleController extends AdminBaseController
{
    ...
    public function index()
    {
        ...
        $where      = ['delete_time' => 0];
        $categoryTree        = $portalCategoryModel->where($where)->select();
    ...
    public function edit()
    {
        ...
        $post_categories = implode(',', $postCategories);
        $postCategoryIds = implode(',', array_keys($postCategories));

        $nav_id =$PortalCategoryModel->where('id',$postCategoryIds)->value('parent_id');
        //查询企业栏目
        $cs = db('navigation')
            ->alias('n')
            ->join('cmf_company c','n.cid = c.id')
            ->where(['n.id'=>$nav_id])
            ->field('n.nav_name,c.name')->find();
