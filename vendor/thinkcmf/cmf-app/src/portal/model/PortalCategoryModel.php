...
//        }
     ...
     * 获取分类数组
     */
    public function andyGetCateArr($parentId=0){
        $where = ['delete_time' => 0];
        if (!empty($parentId)) {
     ...
     * 获取图书分类
     */
    public function portalGetBookCate() {
        $data=Cache::get('bookcate');
        //dump($data);die;
        if (empty($data)){
            $where = [
                'parent_id'=> 4,
            ...
            $data = $this->field('id,name,more')->order("list_order ASC")->where($where)->select()->toArray();
            //将分类键值与分类ID组合数组缓存起来
            $dcate=array();
            foreach ($data as $k=>$v){
                array_push($dcate,$v['id']);
            }
            //dump($data);die;
            Cache::set('dcate',$dcate,360000);//存储一百小时
            Cache::set('bookcate',$data,360000);//存储一百小时
        }
        return $data;
    }
    
    public function getCateIndex($cid){
        $dcate=Cache::get('dcate');
        //dump($dcate);die;
        if ($dcate){
            foreach ($dcate as $k=>$v){
                if ($v == $cid){
                    return $k;
                }
            }
     ...
     * 获取首页分类
     */
    public function portalIndexCate() {
        $data=Cache::get('indexcate');
        if (empty($data)){
            $where = [
                'parent_id'=> 2,
                'delete_time' => 0
            ...
            $data = $this->field('id,name,more')->order("list_order ASC")->where($where)->select()->toArray();
            //dump($data);die;
            Cache::set('indexcate',$data,360000);//存储一百小时
        }
        return $data;
    }
